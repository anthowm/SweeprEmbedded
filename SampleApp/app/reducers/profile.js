  ...
  switch (action.type) {
    ...
    case 'SET_USER_SKILL_LEVEL_REQUESTED':
      return _.assign({}, state, {
        consumerUser: _.assign({}, state.consumerUser, { skillLevel: action.skillLevel })
      });
...
export const ProfileSelector = {
  ...
  userProfileAsArray: (state) => {
    ...
    const fullNameItem = _.assign({}, _.first(_.filter(fieldLabels, { field: 'fullName' })), {
      value: state.consumerUser.firstName + ' ' + state.consumerUser.lastName,
      key: 'key-fullName'
    });

    // Creates fullName item.
    const connectedNetwork = _.assign({}, _.first(_.filter(fieldLabels, { field: 'connectedNetwork' })), {
      value: currentHousehold && currentHousehold.wifiSSID || 'Unavailable',
      key: 'key-connectedNetwork'
    });

    // Creates general user profile items.
    const generalProfileItems = _.map(state.consumerUser, (value, key) => {
  ...
  */
  ...
  selectedSkillLevel: (state) => {
    return state.consumerUser.skillLevel || 0;
  },
