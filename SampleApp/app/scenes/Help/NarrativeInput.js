...
export class NarrativeInput extends Component {
    ...
    if (!_.isEmpty(this.props.notificationData)) {
      ...
      Alert.alert(
                  ...
                  [
                    ...
                    {text: 'OK', onPress: () =>   this.props.incidentActions.createRemoteIncident(incidentRespnse)},
                  ],
                  {cancelable: false},
      );
  ...
  onSpeechError(e) {
    ...
    const errorStr =
      errorMsg.includes('203') ? 'Ooops. I didnt quite catch that' : errorMsg;
  ...
  doKeyboardSubmit() {
    ...
    this.props.incidentActions.createIncident(this.state.keyboardText,householdID);
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (!_.isEmpty(nextProps.incident.lastCreatedIncidentError)) {
      ...
      if(nextProps.incident.lastCreatedIncident.id !== this.props.incident.lastCreatedIncident.id){
       this.processIncidentResponse(nextProps.incident);
      }
    }
}
...
componentDidUpdate(prevProps, prevState) {
  if (prevState.lastIncident.id != this.state.lastIncident.id) {
    if (!_.isEmpty(this.state.narrative )) {
      this.props.navigation.navigate('IssueClarification', { term: this.state.narrative });
...
processIncidentResponse(incident) {
  ...
  renderErrorText() {
    return (
      <View style={[error.container]}>
        <Text style={[Paragraphs.standard, Paragraphs.error, styles.textCenter]}>
          {this.state.error}
        </Text>
      </View>
    );
  ...
  renderActivityIndicator() {
    return !this.state.activityOver ? (
      <Animated.View style={[activity.container, { opacity: this.state.fadeAnim }]}>
        <ActivityIndicator
  ...
  renderBottomBar() {
    return (
      <View style={[bottom.container]}>
        <TouchableHighlight onPress={this.onMicPress} underlayColor={'transparent'}>
          <Image
        ...
        <TouchableHighlight onPress={this.onKeyboardPress} underlayColor={'transparent'}>
          <Image
    ...
    return (
      <View style={voice.container}>
        <View style={voice.description}>
          <Text style={[Titles.main, styles.textCenter]}>How can I help you?</Text>
        </View>
        ...
        <View style={voice.result}>
          {this.state.results.map((result) => {
            ...
            return (
              <Text key={k} style={[Paragraphs.standard, styles.textCenter, styles.resultText]}>
                {result}
              </Text>
            );
          })}
          {_.isEmpty(this.state.results) && (
            <Text style={[Paragraphs.standard, Paragraphs.placeholder, styles.textCenter, styles.startText]}>
  ...
  renderKeyBoardView() {
    return (
      <KeyboardAwareScrollView contentContainerStyle={inputScreenContainer.container}>
