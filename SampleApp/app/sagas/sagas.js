...
function* registerHousehold(action) {
  const response = yield sweeprCloud.createNewHouseHold(
    { wifiSSID: action.householdSSID,
      wifiBSSID:action.householdBSSID},
    action.userID
  );
...
function* fetchHouseholdServices(action) {
  let pageNumber = 0;
  const response = yield sweeprCloud.getHouseHoldServices(pageNumber, action.householdID);
  yield put({ type: 'FETCH_HOUSEHOLD_SERVICES_SUCCEEDED', response });
}

function* addServiceToHousehold(action) {
  const response = yield sweeprCloud.addHouseHoldService(action.serviceID, action.householdID);
  yield put({ type: 'ADD_HOUSEHOLD_SERVICE_SUCCEEDED', response });
}

function* removeServiceFromHousehold(action) {
  const response = yield sweeprCloud.deleteHouseholdService(action.serviceID, action.householdID);
...
function* updateUserSkillLevel(action) {
  ...
  const { id: userID } = state.profile.consumerUser;
...
function* scanForDevices(action) {
  ...
  yield put({ type: 'SYNC_DEVICES_REQUESTED', response, householdID: action.householdID });
}

function* syncDevices(action) {
  const houseHoldId = action.householdID;
  const response = yield sweeprCloud.syncDeviceScan(action.response.identifier, houseHoldId);
...
function* syncResolutionAttempt(action) {
  ...
  const response = yield sweeprCloud.saveResolutionCloud(houseHoldId, action.response.audit);
...
function* createIncident(action) {
  try {
    ...
    yield put({ type: 'CREATE_INCIDENT_SUCCEEDED', incidentResponse: response });
  } catch (e) {
...
function* publishDeviceResponseForIncident(action) {
  ...
  if (incidentType === 'IncidentTypeDiagnostic') {
    ...
    yield put({ type: 'RESPOND_WITH_DEVICE_REQUESTED_SUCCEEDED', response: response });
  } else {
...
function* unSubscribeToIncident(action) {
  const response = yield sweeprResolution.unsubcribeToIncidentWithIncidentID(action.incidentID);
...
function* acknowledgeRemoteIncident(action) {
  ...
  yield put({ type: 'RESOLUTION_SUBSCRIBE_SUCCEEDED', ackResponse: ackResponse });
}
...
function* acknowledgeIncident(action) {
  ...
  } else { // this is a standard resolution type,  or a handover type
...
function* fetchSpeedTest(action) {
  ...
  yield put({ type: 'SPEED_TEST_SUCCEEDED', response: response[0], householdID: action.householdID });
}
