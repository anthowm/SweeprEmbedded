...
class HouseholdRegistration extends Component {
  ...
  selectNetwork() {
    this.props.actions.registerHousehold(this.props.consumerUser.id, this.props.householdSSID, this.props.householdBSSID);
    this.props.navigation.navigate('Main');
  }

  render() {
    const missingSSID = _.isEmpty(this.props.householdSSID);
    const missingUUID = _.isUndefined(this.props.consumerUser.id);
    const haveNetwork = !missingSSID && !missingUUID;
    const text = haveNetwork ? 'Is this the home network you\nwould like to manage?' : "You dont seem to be connected to a wifi network.\n Please connect and relaunch."
          ...
          <View style={styles.networkBox}>
            <View>
              <View style={styles.homeNetworkIconWrapper}>
                <Image source={homeNetworkIcon} style={styles.homeNetworkIcon} />
              </View>
              <Text style={styles.networkTitle}>{this.props.householdSSID}</Text>
              <View style={styles.customButtonWrapper}>
                <Button isDisabled = {!haveNetwork} style={[button, styles.swipeForHelp]} onPress={this.selectNetwork}>
