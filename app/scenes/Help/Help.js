...
import { button, buttonText, button_secondary, buttonText_secondary } from '../../Styles/Form';
...
export class HelpWithDevice extends Component {
  constructor(props) {
    ...
    this.renderConnectionSpeedCheck = this.renderConnectionSpeedCheck.bind(this);
    this.renderDoorbellSpeedCheck = this.renderDoorbellSpeedCheck.bind(this);
    this.renderDoorbellDeviceWorking = this.renderDoorbellDeviceWorking.bind(this);
    ...
    this.postIncidentAndResolutionAttempts = this.postIncidentAndResolutionAttempts.bind(this);
  ...
  componentWillMount() {
    ...
    this.setState({
      ...
      filtered: this.filteredValues(this.props.navigation.state.params.term, this.props.connected),
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      ...
      filtered: this.filteredValues(nextProps.navigation.state.params.term, nextProps.connected),
  ...
  sanitisedResolutionAttempts(list) {
    return _.map(list, (attempt) => {
      return _.pick(attempt, ['status', 'dateAttempt', 'speedTestResult', 'auditSummary', 'step']);
    });
  }
  ...
  pushResolutionAttempt(item) {
    let resolutionAttempts = this.resolutionAttempts ? this.resolutionAttempts.slice(0) : [];

    let template = { dateAttempt: Date.now(), status: null, step: resolutionAttempts.length + 1 };
  ...
  isGamingDevice(item) {
    return item.origin.deviceType === 'playstation' || item.origin.deviceType === 'xbox';
  }
  ...
  renderActivityScreen(label) {
    return (
      <View style={[styles.container, styles.centeredContent, styles.content]}>
        <Text style={[styles.label, styles.textCenter, styles.stepSectionMargin]}>{label}</Text>
        <ActivityIndicator
  ...
  renderDevicesList() {
    return (
      <ScrollView style={[styles.container, styles.content]}>
        ...
        />
        <Text style={[styles.label, styles.stepSectionMargin]}>What device did you mean?</Text>
        <FlatList
          data={this.state.filtered}
          style={{ overflow: 'visible' }}
          renderItem={({ item }) => <DeviceListItem onPress={this.startProblemFlow} item={item} />}
        />
  ...
  renderDeviceCheck() {
    ...
    return this.renderActivityScreen(`Checking ${this.state.item.name} connectivity...`);
  }
  ...
  renderDoorbellSpeedCheck() {
    ...
    return this.renderActivityScreen(`Testing ${this.state.item.name} speed...`);
  }
  ...
  renderDoorbellDeviceWorking() {
    return (
      <View style={[styles.container, styles.centeredContent, styles.content]}>
        <Text style={[styles.label, styles.textCenter, styles.stepSectionMargin]}>
  ...
  renderRemoteServersCheck() {
    ...
    return this.renderActivityScreen(`Checking ${this.state.gamingService} service..`);
  }
  ...
  renderPrioritiseConnection() {
    return (
      <View style={[styles.container, styles.centeredContent, styles.content]}>
        ...
        <Text style={[styles.label, styles.textCenter, styles.stepSectionMargin]}>
          Would you like us to give this activity priority on your home internet?
        </Text>
  ...
  render3rdPartyError() {
    this.pushResolutionAttempt({
      ...
      auditSummary: `Notify user of service (${this.state.gamingService}) experiencing trouble.`
    });
    return (
      <View style={[styles.container, styles.centeredContent, styles.content]}>
        <Text style={[styles.label, styles.textCenter, styles.stepSectionMargin]}>
  ...
  postIncidentAndResolutionAttempts() {
    ...
    if (!this.incidentReported) {
      if (this.attemptsPosted) {
        ...
        let payloadList = this.sanitisedResolutionAttempts(this.resolutionAttempts);
        performNextAction(payloadList);
      } else {
        console.warn('No incident ID to add steps to.', this.resolutionAttempts);
      }
    } else {
